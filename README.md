...
When you run mobile campaigns, boost of new app installs are often one of the main goals. Easy to install Tracking SDK will enable campaign app installs tracking and reporting in Adform platform without need to deal with 3rd party SDKs and invest tons of time into that.
...
* Then in the dependency group we need to specify that we will be using `AdformTrackingSdk`, and also add `Google Play Ads` service with `Protobuf` library.
        ...
        dependencies {
    		...
    		compile 'com.adform.tracking.sdk:adform-tracking-sdk:1.1.3'
